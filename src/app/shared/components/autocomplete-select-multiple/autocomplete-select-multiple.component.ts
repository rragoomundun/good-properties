...
@Component({
  selector: 'app-autocomplete-select-multiple',
  standalone: true,
  imports: [TranslateModule, ReactiveFormsModule],
  templateUrl: './autocomplete-select-multiple.component.html',
  styleUrl: './autocomplete-select-multiple.component.scss',
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => AutocompleteSelectMultipleComponent),
      multi: true,
    },
  ],
  viewProviders: [
...
export class AutocompleteSelectMultipleComponent
  implements ControlValueAccessor, OnInit
{
  ...
  setValue(): void {
    if (
      this.selectedItems.includes(this.availableItems[this.selected]) === false
    ) {
      this.selectedItems.push(this.availableItems[this.selected]);
    }
          ...
          if (this.selected > 0) {
            ...
            const selectedEl = <HTMLLIElement>(
              document.querySelector(
                'app-autocomplete-select-multiple #dropdown .selected',
              )
            );
